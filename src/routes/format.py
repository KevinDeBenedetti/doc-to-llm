    ...
    for line in lines:
        ...
        if '[' in line and '](' in line and not line.strip().startswith('!['):
            ...
            for text, url in matches:
                if not url.startswith('http') and not url.startswith('#') and not url.endswith('.md'):
                    if not url.endswith('/'):
...
async def enhance_content_with_ai(content: str) -> str:
    ...
    try:
        ...
        if health["status"] != "healthy":
            print(f"Attention: Service AI non disponible ({health.get('error', 'Unknown error')}), amélioration ignorée")
            return content
...
async def format_doc(file: UploadFile = File(...), enhance: bool = True, clean: bool = True):
    try:
        # Vérifier le type de fichier
        if not file.filename.endswith('.md'):
            raise HTTPException(status_code=400, detail="Le fichier doit être un fichier markdown (.md)")
        ...
        raise HTTPException(status_code=500, detail=f"Erreur lors du formatage : {str(e)}")
...
async def format_vitepress_markdown_streaming(content: str) -> AsyncGenerator[str, None]:
    """Formate le contenu VitePress en streaming avec des chunks de progression"""
    yield json.dumps({"status": "starting", "message": "Début du formatage VitePress..."}) + "\n"
...
async def add_vitepress_utilities_streaming(content: str) -> AsyncGenerator[str, None]:
    ...
    yield json.dumps({"status": "adding_utilities", "progress": 40, "message": "Ajout des utilitaires VitePress..."}) + "\n"
    ...
    if not has_frontmatter:
        ...
        yield json.dumps({"status": "utilities", "message": "Frontmatter ajouté"}) + "\n"
    ...
    if '[[toc]]' not in content_str.lower():
        for i, line in enumerate(enhanced_lines):
            if line.startswith('# ') and i > 0:
                ...
                enhanced_lines.insert(i + 2, '[[toc]]')
                enhanced_lines.insert(i + 3, '')
                yield json.dumps({"status": "utilities", "message": "Table des matières ajoutée"}) + "\n"
                break
...
async def process_document_streaming(content: str, clean: bool = True, enhance: bool = True) -> AsyncGenerator[str, None]:
...
async def format_doc_stream(file: UploadFile = File(...), enhance: bool = True, clean: bool = True):
    try:
        # Vérifier le type de fichier
        if not file.filename.endswith('.md'):
            raise HTTPException(status_code=400, detail="Le fichier doit être un fichier markdown (.md)")
        ...
        raise HTTPException(status_code=500, detail=f"Erreur lors du formatage streaming : {str(e)}")
...
async def format_doc_text_stream(request: DocumentRequest):
    try:
        # Créer le générateur de streaming
        async def generate():
            async for chunk in process_document_streaming(request.content, request.clean, request.enhance):
                yield chunk
...
async def format_doc_markdown(file: UploadFile = File(...), enhance: bool = True, clean: bool = True):
    try:
        # Vérifier le type de fichier
        if not file.filename.endswith('.md'):
            raise HTTPException(status_code=400, detail="Le fichier doit être un fichier markdown (.md)")
